...
void dijkstra(int source){
	...
	q.push({0,source});
	while(!q.empty()){
		int current_source=q.top().second;
		int current_dist=q.top().first;
		q.pop();
		for(pair<int,int> child:v[current_source])
		{
				int new_dist=child.second+current_dist;
				if(new_dist<dist[child.first])
				{
					dist[child.first]=new_dist;
					q.push({new_dist,child.first});
				}
		}
	}
}
